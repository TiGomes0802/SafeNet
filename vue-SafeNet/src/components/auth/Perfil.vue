                ...
                <div class="max-w-6xl mx-auto bg-white rounded-xl shadow-lg p-8 mt-10 flex flex-col items-center">
                    ...
                    <template v-if="user?.type === 'J'">
                        <div class="flex flex-wrap gap-8 justify-center mb-6">
                            ...
                            <div class="flex flex-col items-center">
                                <span class="text-xl font-semibold text-orange-600">{{ user?.streak }} {{
                                    user.streakFeita ? '🔥' : '🌡️' }}</span>
                                <span class="text-gray-500 text-sm">Streak</span>
                            </div>
                            <div v-if="storeAuth.user?.username === route.params.username"
                    ...
                    <div class="mt-6 flex gap-4">
                        <button class="btn-soft" @click="() => router.push({ name: 'UpdatePerfil' })"
                            v-if="isOwnProfile">Editar
                            Perfil</button>
                        <button class="btn-soft" @click="adicionarAmigo" :disabled="!loadingAdicionar"
                            v-if="!isOwnProfile && !user?.isFriend && !user?.isHaveRequest && user?.type == 'J'">Adicionar
                            Amigo</button>
                    </div>
                </div>
            </div>
